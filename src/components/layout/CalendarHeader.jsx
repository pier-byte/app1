...
export default function CalendarHeader({ mode, onModeChange, selectedDate, onPrev, onNext, onToday, weekStrip }) {
  return (
    <header className="border-b border-separator-opaque bg-canvas/95 backdrop-blur-dialog shrink-0">
      <div className="max-w-[1100px] mx-auto w-full px-4 pt-3">
        <div className="flex items-center justify-between gap-3 mb-2">
          <div>
            <h1 className="text-[25px] font-semibold tracking-tight capitalize">
              {format(selectedDate, 'MMMM yyyy', { locale: it })}
            </h1>
            <button onClick={onToday} className="text-accent text-[13px] min-h-8">
              Vai a oggi
            </button>
          </div>
              ...
              {['week', 'month'].map((v) => (
                <button
                  ...
                  className={cn(
                    ...
                    mode === v ? 'bg-accent text-white' : 'text-label-secondary'
                  )}
                >
